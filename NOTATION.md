...
作业 1 现在同时包含**两份数值等价、可独立运行**的实现：

1. **`no_einstein`（版本 1）**：不使用爱因斯坦 notation。矩阵乘用 `@` / `torch.matmul`，
   维度整理用 `reshape` / `view` / `transpose` / `permute`，归一化用 `torch.mean` /
   `torch.sum`，取值用高级索引。
2. **`einstein`（版本 2）**：只用**统一的爱因斯坦 API `einx`**。缩合用 `einx.dot`，
   结构变换用 `einx.id`，归一化与索引用 `einx.softmax` / `einx.logsumexp` /
   `einx.get_at` / `einx.mean` / `einx.multiply`。**不 import `einops`，也不用
   `torch.einsum`。**

> 选型依据：`einx` 是统一 API，已经覆盖了 `einops`（结构变换）与 `einsum`（代数缩合）
> 的能力，并额外提供逐元素、归一化、索引等算子，所以版本 2 只依赖 `einx`。
> 对应关系：`einx.id` ≈ `einops.rearrange`，`einx.dot` ≈ `einops.einsum` /
> `torch.einsum`。

两份实现共享同一套公开 API，官方测试套件（`tests/`）对两者都 **100% 通过**。
...
| 操作 | `no_einstein`（版本 1） | `einstein`（版本 2，纯 einx） |
| :-- | :-- | :-- |
...
* **轴名即文档**：版本 2 的每个缩合都能直接对照数学公式，且自动支持任意前置 batch
  维度（`...`）。
* **纯 einx**：版本 2 只 import `einx`，不再依赖 `einops`，也不用 `torch.einsum`。
* **rank 对齐**：`einx.dot` **不会**跨秩隐式广播，所以 RoPE 里用
  `_broadcast_leading` 配合 `einx.id` 把位置角度的前置维度显式对齐到 q/k。
* **数值等价**：两版都做了数值稳定处理（Softmax / log-sum-exp 先减最大值），
  输出在官方容差内一致，都能通过快照测试。
* **转发层**：`cs336_basics.model.layers` 等只是 import 时的转发，选择在进程启动时
  一次性完成，不会引入运行时开销。
* **默认版本**：如果未设置 `CS336_NOTATION`，默认使用 `einstein`（课程推荐做法）。
